<?php
// arquivo: aai_form_novo.php — Página de cadastro SEM sidebar/topbar, tema EMOPS
// Usa a conexão existente (MySQLi) via conn.php
session_start();
require_once __DIR__ . '/conn.php'; // cria $conn (mysqli)
...
function clip(?string $s, int $max = 10000): string { return mb_substr((string)$s, 0, $max); }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  $val['atividades_realizadas'] = clip((string)($_POST['atividades_realizadas'] ?? ''), 10000);
  $val['atividades_previstas']  = clip((string)($_POST['atividades_previstas'] ?? ''), 10000);
  $val['pontos_relevantes']     = clip((string)($_POST['pontos_relevantes'] ?? ''), 10000);
